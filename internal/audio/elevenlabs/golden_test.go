...
// matches the checked-in golden fixture. Finding #10: external golden file catches
// simultaneous Default+tts.go drift that a self-referential invariant test misses.
func TestGolden_DefaultBody(t *testing.T) {
	cfg := elevenlabs.Config{APIKey: "test-key", VoiceID: "test-voice"}
	body, path := captureElevenLabsBody(t, cfg, audio.TTSOptions{})
	...
	wantNorm := canonicalJSON(t, wantMap)
	gotNorm := canonicalJSON(t, gotMap)
	if gotNorm != wantNorm {
